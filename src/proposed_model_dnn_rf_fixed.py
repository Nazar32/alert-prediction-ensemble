#!/usr/bin/env python3
"""
proposed_model_dnn_rf_fixed.py
==============================
Copy of ``experiments/proposed_model_dnn_rf.py`` with a single fix:

- Return probabilities aligned with the cleaned test frame (``test_df_clean``),
  so metrics that use ``y_prob`` (e.g., ROC AUC) are computed on correctly
  aligned arrays.

The original file is intentionally left unchanged for reproducibility.

Run from the project root:
    python experiments/proposed_model_dnn_rf_fixed.py
"""
...
def run_proposed_dnn_rf(
    ...
    y_prob      ensemble probabilities aligned to y_true (post isotonic calibration)
...
def main():
    ...
    # Per-region precision summary (kept identical to original)
    ...
    # Save
    out_path = os.path.join(RESULTS_DIR, "proposed_dnn_rf_fixed_results.csv")
    pd.DataFrame([{"model": "Proposed_DNN_RF_Fixed", **metrics}]).to_csv(out_path, index=False)
