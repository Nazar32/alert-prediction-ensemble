#!/usr/bin/env python3
"""Run LightGBM with per-region calibration. Save probs to disk."""
import sys, os
...
clf = lgb.LGBMClassifier(n_estimators=300, max_depth=8, learning_rate=0.05,
...
print(f"LightGBM+per-region: Prec={m['precision']:.4f}  Rec={m['recall']:.4f}  "
      f"F1={m['f1']:.4f}  AUC={m['roc_auc']:.4f}")
pd.DataFrame([{"model": "LightGBM+per-region", **m}]).to_csv(
    "results/lgb_per_region_results.csv", index=False)
print("Saved results/lgb_per_region_results.csv")
